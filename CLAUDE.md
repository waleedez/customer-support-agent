...
CORS (`langgraph.json`'s `"http.cors"` → baked in as `ENV LANGGRAPH_HTTP`, read by `langgraph_api` at startup into `CORS_CONFIG`, applied via Starlette's `CORSMiddleware` — traced through `.venv/Lib/site-packages/langgraph_api/{server.py,config/__init__.py}` to confirm, not guessed) only solves *cross-origin* blocking. It does **not** solve mixed-content blocking: Studio (`https://smith.langchain.com`) is HTTPS and browsers refuse to even attempt a fetch to a plain `http://` resource, CORS headers or not. That's what `docker-compose.override.yml` + `Caddyfile` are for — a TLS-terminating reverse proxy in front of `langgraph-api`, kept in a separate override file specifically so regenerating `docker-compose.yml` never deletes it.

**History**: this started as a self-signed cert straight on the IP (`scripts/generate_self_signed_cert.sh` + `nginx/nginx.conf`), which worked but meant every single user had to manually visit the IP and click through their browser's untrusted-cert warning before Studio's background fetches would succeed — a second person hitting `ERR_CERT_AUTHORITY_INVALID` made that not worth repeating. It's now Caddy + a real Let's Encrypt cert for `cs-agent.duckdns.org` (DNS already verified to resolve to `150.230.245.145` via `nslookup` before building this) — `nginx/` and `scripts/generate_self_signed_cert.sh` were deleted rather than left as dead config. Don't resurrect the self-signed path without a reason; it's strictly worse now that a real domain exists.

Caddy specifics: `caddy_data`/`caddy_config` are named Docker volumes (not bind mounts) holding the issued cert + ACME account key — `docker compose down` is safe, `docker compose down -v` deletes them and forces re-issuance on next `up` (Let's Encrypt rate-limits to 5 duplicate certs/week/domain, so don't `-v` casually). Both port 80 (ACME HTTP-01 challenge) and 443 must be reachable from the public internet, not just 443. `flush_interval -1` in `Caddyfile` disables response buffering for Studio's SSE run-streaming (mirrors nginx's old `proxy_buffering off`).

Validated without touching the live domain/ACME: `docker run --rm -v ./Caddyfile:/etc/caddy/Caddyfile:ro caddy:2-alpine caddy validate --config /etc/caddy/Caddyfile --adapter caddyfile` confirms syntax; actual proxy mechanics were verified by running a *separate* scratch Caddyfile using `tls internal` (Caddy's local-test CA) against `localhost`, joined to the compose network via `docker run --network <project>_default`, hitting `/ok` and `/assistants/search` through it — never by pointing real Caddy/ACME at `cs-agent.duckdns.org` from this dev machine, since Let's Encrypt's HTTP-01 validator would connect to the real `150.230.245.145` (not this sandbox) and fail, plus needlessly risking the domain's real rate limit.

On Windows dev boxes specifically, ports 80/443 may be in Windows' *own* excluded TCP port range (Hyper-V/WSL dynamic ports) independent of Docker — `netsh interface ipv4 show excludedportrange protocol=tcp` shows this; it's a host quirk, not a compose/proxy bug, and doesn't reproduce on a normal Linux host. Local testing therefore used temporary alternate ports (`8443`/`8080`), never the committed `443`/`80`.

Git-Bash-on-Windows note: `docker run` args starting with `/etc/...` etc. get silently rewritten into Windows paths (e.g. `caddy validate --config /etc/caddy/Caddyfile` → `C:/Program Files/Git/etc/caddy/Caddyfile`) unless you either use `MSYS_NO_PATHCONV=1` (but that also disables conversion of *actual* host paths elsewhere in the same command, e.g. `-v` source paths — don't reach for it blindly) or the double-leading-slash trick (`//CN=...`) for args where the target program tolerates a leading empty path component. Pick per-argument, not per-command.
